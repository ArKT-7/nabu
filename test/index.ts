...
<head>
	...
	pre.result-field {
	    background: linear-gradient(135deg, #f0f4f8, #e2e8f0); 
	    padding: 20px 20px 20px; /* Adjust bottom padding */
	    border-radius: 15px;
	    overflow-y: auto; 
	...
	pre.result-field::-webkit-scrollbar-thumb {
	    ...
	    height: 50px; /* Set a fixed height for the scrollbar thumb */
...
async function connectDevice() {
    ...
    // let resultField = document.querySelector(".result-field"); // Commenting out result field reference
    ...
    let win_chk = await device.getVariable("partition-type:win");
    let esp_chk = await device.getVariable("partition-type:esp"); 
    let userdata_size = await device.getVariable("partition-size:userdata");
    let win_size = await device.getVariable("partition-size:win");
    let esp_size = await device.getVariable("partition-size:esp");
    ...
    status += `<br />Bootloader status: ${bootloaderStatusText}`; // Keep Bootloader status without extra spacing
	
	// Check partitions availability and sizes
	let sizeInfo = ''; // Initialize a variable to collect size info
	...
	if (await checkPartition("win", win_chk, win_size)) {
	    sizeInfo += `<p style="margin: 0; margin-bottom: 0px;">Windows Partition : ${getSizeString(win_size)}</p>`;
	}
	if (await checkPartition("esp", esp_chk, esp_size)) {
	    sizeInfo += `<p style="margin: 0; margin-bottom: 0px;">ESP Partition : ${getSizeString(esp_size)}</p>`;
	}
	
	if (sizeInfo) {
	    status += `<br />${sizeInfo}`; // Add size info to status only if available
	}
	...
	// Function to check a partition's availability
	async function checkPartition(partitionName, partitionChk, partitionSize) {
	    return partitionChk; // Return true if the partition is available
	}
	
	// Function to get size string based on the size value
	function getSizeString(hex) {
	    const size = parseInt(hex, 16); // Convert hex to integer
	    const sizeInGB = size / (1024 ** 3);
	    const sizeInMB = size / (1024 ** 2);
	
	    // Return the size in appropriate format
	    if (sizeInGB < 1) {
	        return `${sizeInMB.toFixed(2)} MB`; // Show in MB if under 1 GB
	    }
	    return `${sizeInGB.toFixed(2)} GB`; // Show in GB otherwise
	}
	
	// Function to write messages to the result field
	// function writeToResultField(message, resultField) {
	//     resultField.innerHTML = `${message}<br />` + resultField.innerHTML; // Prepend new message
	// }
...
async function sendFormCommand(event) {
    ...
    if (!command) {
        resultField.textContent = "\nPlease enter a command!\n" + resultField.textContent; // Prepend error message
    ...
    try {
        ...
        // Use backticks for error message as well
        resultField.textContent = `\nError executing command: ${error.message}\n` + resultField.textContent; 
        alert(`Error executing command: ${error.message}`);
    } finally {
        inputField.value = ""; // Clear the input field after execution
        await checkDeviceConnection(); // Check device connection after the command
    }
}
...
async function wipepart() {
	...
	// Step 2: Execute additional erase commands
        await device.runCommand("erase:metadata");
        await device.runCommand("erase:userdata");

        // Step 3: Download and flash userdata to partition "userdata"
